...
# ==== Globals for concurrency & sequences (no hard limits) ====
# Track active auto-sequences per user
active_sequences = {}
# Track temporary status/notification message IDs per user
message_ids = {}
# Track recent renaming operations by file_id to avoid double-processing
renaming_operations = {}
# Thread pool for CPU-bound or blocking tasks (e.g., FFmpeg, image ops)
...
async def send_log(client, message, orig_name, new_name, path, media_type, ph_path):
    """Send the renamed file to the log channel with details."""
    try:
        ...
        if not log_chat:
            # Fallback: don't crash if not configured
            return
...
async def run_ffmpeg_async(metadata_command):
    """Run FFmpeg in thread pool without semaphore limit."""
    def _run_ffmpeg():
        import subprocess
...
async def concurrent_download(client, message, renamed_file_path, progress_msg):
    """Handle downloading without artificial semaphore limits."""
    try:
...
async def concurrent_upload(client, message, path, media_type, caption, ph_path, progress_msg):
    """Handle uploading without artificial semaphore limits."""
    try:
        if media_type == "document":
            await client.send_document(
                message.chat.id,
...
async def auto_rename_file_concurrent(client, message, file_info):
    """Main concurrent function without global processing semaphore."""
    path = None
    ph_path = None
    try:
        user_id = message.from_user.id
