...
def premium_check():
    def decorator(func):
        ...
        async def wrapper(client, message, *args, **kwargs):
            ...
            # If user can rename, increment counter (only for non-premium users)
            ...
            if not is_premium:
                ...
                if remaining_files <= 10:  # Warning when close to limit
                    keyboard = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🌟 Upgrade to Premium", callback_data="premium_info")]
                    ])
                    await message.reply_text(
...
# Modified send_log function to send the actual renamed file to log channel
async def send_log(client, message, old_name, new_name, file_path, media_type, ph_path=None):
    """
    Send the renamed file and logging information to the log channel
    """
    try:
        user = message.from_user
        is_premium = await Botskingdom.is_premium_user(user.id)
        premium_status = "🌟 Premium" if is_premium else "👤 Free"
        
        user_info = f"{premium_status} User: {user.first_name} (@{user.username or 'None'}) | ID: {user.id}"
        rename_info = f"📝 Renamed: {old_name} → {new_name}"
        
        # Create caption for the log channel
        log_caption = f"{user_info}\n{rename_info}\n⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        
        # Send the actual file to log channel based on media type
        if media_type == "video":
        ...
        # Also send a text summary for quick overview
        summary_msg = f"✅ File Renamed Successfully\n{user_info}\n{rename_info}"
        await client.send_message(Config.LOG_CHANNEL, summary_msg)
        
    except Exception as e:
        # If logging fails, send error info
        try:
            error_msg = f"❌ Log Error for user {user.id}: {str(e)}"
            await client.send_message(Config.LOG_CHANNEL, error_msg)
        except:
            pass  # If even error logging fails, just ignore
...
async def start_sequence(client, message: Message):
    ...
    # Check premium status for sequence feature
    is_premium = await Botskingdom.is_premium_user(user_id)
    if not is_premium:
        # Check if user has enough remaining files for sequence
        files_today = await Botskingdom.get_user_usage_today(user_id)
        remaining = Config.FREE_USER_DAILY_LIMIT - files_today
        
        if remaining < 5:  # Require at least 5 files remaining for sequence
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🌟 Upgrade to Premium", callback_data="premium_info")]
            ])
            await message.reply_text(
...
async def auto_rename_files(client, message):
    ...
    if not is_premium:
        ...
        # Send status message for free users
...
async def end_sequence(client, message: Message):
    ...
    if user_id not in active_sequences:
        ...
        # Check if user has enough remaining files
        is_premium = await Botskingdom.is_premium_user(user_id)
        if not is_premium:
            files_today = await Botskingdom.get_user_usage_today(user_id) 
            remaining = Config.FREE_USER_DAILY_LIMIT - files_today
        ...
        if not file_list:
            ...
            await message.reply_text(f"Sᴇǫᴜᴇɴᴄᴇ ᴇɴᴅᴇᴅ. ({status})\nNᴏᴡ sᴇɴᴅɪɴɢ ʏᴏᴜʀ {count} ғɪʟᴇ(s) ʙᴀᴄᴋ ɪɴ sᴇǫᴜᴇɴᴄᴇ...!!")
            
            # Increment file count for each file if not premium
            if not is_premium:
            ...
            if not is_premium:
                ...
                await message.reply_text(
                    f"✅ Aʟʟ {count} ғɪʟᴇs sᴇɴᴛ sᴜᴄᴄᴇssғᴜʟʟʏ!\n"
...
async def auto_rename_file_concurrent(client, message, file_info):
    """
    MAIN CONCURRENT FUNCTION - Enhanced with premium status tracking
    ...
    async with processing_semaphore:  # Limit overall concurrent processing
        try:
            ...
            try:
                ...
                # Show completion status based on premium status
                if is_premium:
                ...
                # MODIFIED: Send log with the actual file to log channel
                # This sends the renamed file to the log channel instead of just a text message
                await send_log(client, message, file_name, renamed_file_name, path, media_type, ph_path)
...
# Add callback handlers for premium buttons
