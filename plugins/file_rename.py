...
# ==== Enhanced Globals for True Concurrency ====
...
# Enhanced thread pool for CPU-bound tasks with more workers
thread_pool = ThreadPoolExecutor(max_workers=(os.cpu_count() or 4) * 2)

# Global semaphore for download/upload operations per user (allow multiple users simultaneously)
user_semaphores = {}
...
def get_user_semaphore(user_id):
    """Get or create a semaphore for a specific user to limit their concurrent operations"""
...
async def auto_rename_files(client, message):
    """Enhanced main handler with true concurrent processing"""
    user_id = message.from_user.id
    ...
    if operation_key in renaming_operations:
        ...
        if elapsed_time < 5:  # Reduced from 10 to 5 seconds for faster processing
            return
    ...
    # Process file immediately with enhanced concurrency - NO BLOCKING WAITS
    asyncio.create_task(auto_rename_file_concurrent(client, message, file_info))

@Client.on_message(filters.command("end_sequence") & filters.private)
async def end_sequence(client, message: Message):
    """Enhanced sequence ending with concurrent processing"""
    user_id = message.from_user.id
    ...
    if not is_premium:
        ...
        await message.reply_text(
            f"✅ Aʟʟ {count} ғɪʟᴇs ᴘʀᴏᴄᴇssᴇᴅ ᴄᴏɴᴄᴜʀʀᴇɴᴛʟʏ!\n"
            f"📊 Remaining today: {new_remaining}"
        )
    else:
        await message.reply_text(f"✅ Aʟʟ {count} ғɪʟᴇs ᴘʀᴏᴄᴇssᴇᴅ ᴄᴏɴᴄᴜʀʀᴇɴᴛʟʏ!")
...
async def run_ffmpeg_async(metadata_command):
    """Run FFmpeg in thread pool without blocking."""
    def _run_ffmpeg():
        import subprocess
        try:
...
async def concurrent_download(client, message, renamed_file_path, progress_msg, user_semaphore):
    """Enhanced concurrent download with user-specific semaphore"""
    async with user_semaphore:  # Limit concurrent downloads per user
        try:
...
async def concurrent_upload(client, message, path, media_type, caption, ph_path, progress_msg, user_semaphore):
    """Enhanced concurrent upload with user-specific semaphore"""
    async with user_semaphore:  # Limit concurrent uploads per user
        try:
            if media_type == "document":
                await client.send_document(
                    message.chat.id,
...
async def auto_rename_file_concurrent(client, message, file_info):
    """FULLY ENHANCED concurrent function with true multi-user support"""
    path = None
    ph_path = None
    user_semaphore = None
    ...
    try:
        ...
        # Get user-specific semaphore for rate limiting per user
        ...
        episode_number = extract_episode_number(file_name or "Unknown")
        season_number = extract_season_number(file_name or "Unknown")
        audio_info_extracted = extract_audio_info(file_name or "Unknown")
        quality_extracted = extract_quality(file_name or "Unknown")
        ...
        try:
            # Download with user-specific concurrency control
            path = await concurrent_download(client, message, renamed_file_path, download_msg, user_semaphore)
            ...
            # Upload with user-specific concurrency control
            await concurrent_upload(client, message, path, media_type, caption, ph_path, download_msg, user_semaphore)
...
async def cleanup_old_operations():
    """Clean up old renaming operations and user semaphores"""
    while True:
        try:
            ...
            # Clean up unused user semaphores (users not active for 1 hour)
            # This is a simple cleanup - in production you might want more sophisticated tracking
            if len(user_semaphores) > 100:  # If too many semaphores, clear old ones
                user_semaphores.clear()
            
            print(f"Cleaned up {len(old_operations)} old operations and managed semaphores")
...
print("🚀 Enhanced Concurrent File Rename Bot - Ready for Multiple Users!")