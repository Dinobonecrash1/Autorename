...
async def cb_handler(client, query: CallbackQuery):
    ...
    try:
        if data == "home":
            ...
            # FIXED: Handle format template properly to avoid KeyError
            try:
            ...
            # CRITICAL FIX: Escape the format template to prevent formatting errors
            # Replace the format template braces to avoid KeyError when displaying
            display_template = format_template.replace("{", "{{").replace("}", "}}")
            
            try:
                # Try to format with the escaped template
                message_text = Txt.FILE_NAME_TXT.format(format_template=display_template)
            except:
                # If that still fails, use a safe fallback
                message_text = f"""**🏷 Auto Rename Format**

**Current Format:** `{format_template}`
